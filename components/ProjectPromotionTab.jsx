...
export default function ProjectPromotionTab({ items = [], projects = [], marketAnalysisDefaults = {}, onUpdateItem, onOpenSupply, onOpenDistribution, onOpenMarket, onCreateProjectDraft, onOpenProject, syncState }) {
  ...
  return <div style={{ display: "grid", gap: 14 }}>
    ...
    <div className="promotion-page-layout" style={{ display: "grid", gridTemplateColumns: "290px minmax(0, 1fr)", gap: 14, alignItems: "start" }}>
      <aside style={{ ...panelStyle, padding: 10, position: "sticky", top: 88 }}>
        ...
        <div style={{ display: "grid", gap: 7, marginTop: 10, maxHeight: "calc(100vh - 250px)", overflowY: "auto" }}>{visibleItems.map((item) => { const active = String(item.id) === String(selectedItem?.id); const itemPromotion = normalizeProjectPromotion(item.projectPromotion); return <button key={item.id} type="button" onClick={() => setSelectedItemId(item.id)} style={{ padding: 10, textAlign: "left", borderRadius: 7, border: `1px solid ${active ? "#2563eb" : "#cbd5e1"}`, background: active ? "#eff6ff" : "#fff", cursor: "pointer" }}><div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{itemLabel(item)}</div><div style={{ marginTop: 4, color: "#64748b", fontSize: 11 }}>{item.manufacturer || "제조사 미입력"} · {item.permitCompany || "허가사 미입력"}</div><div style={{ marginTop: 6 }}><span style={promotionProgressBadgeStyle(itemPromotion.progressDecision)}>{promotionProgressLabel(itemPromotion.progressDecision)}</span></div></button>; })}{visibleItems.length === 0 && <div style={{ padding: 18, color: "#94a3b8", textAlign: "center", fontSize: 12 }}>조건에 맞는 추진 품목이 없습니다.</div>}</div>
      </aside>
