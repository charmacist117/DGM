...
test("all existing release keys and notices are retained", () => {
  assert.equal(DASHBOARD_CHANGE_SEEDS.length, 47);
  assert.equal(new Set(DASHBOARD_CHANGE_SEEDS.map((seed) => seed.key)).size, 47);
  assert.equal(new Set(DASHBOARD_CHANGE_SEEDS.map((seed) => seed.id)).size, 47);
  assert.equal(DASHBOARD_CHANGE_SEEDS.reduce((sum, seed) => sum + seed.changes.length, 0), 126);
  const logs = append([]);
  assert.equal(logs.length, 47);
  assert.deepEqual(logs.map((log) => log.revision), Array.from({ length: 47 }, (_, index) => String(index + 1)));
...
test("legacy registration flags can suppress deleted notices and the initial notice stays initial-only", () => {
  ...
  assert.equal(append(events).length, 48);
});
