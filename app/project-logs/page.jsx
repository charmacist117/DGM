...
export default function ProjectLogsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!dataResp.ok || !dataPayload.ok) throw new Error(dataPayload.error || dataPayload.message || "조회 실패");
        if (disposed) return;
  ...
  const visibleLogs = useMemo(
    () => [...logs].filter((l) => isAdmin || !l.hiddenForManager).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)),
    [logs, isAdmin]
  );
  ...
  const persist = async (nextLogs) => {
    ...
    try {
      ...
      if (!resp.ok || !payload.ok) throw new Error(payload.error || payload.message || "저장 실패");
      setLogs(nextLogs);
    } catch (e) {
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", padding: 20 }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gap: 12 }}>
        <div style={{ ...cardStyle, padding: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={() => router.push("/")} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #cbd5e1", background: "#fff" }}>돌아가기</button>
        </div>
        {error && <div style={{ ...cardStyle, padding: 10, color: "#dc2626" }}>{error}</div>}
        <div style={{ ...cardStyle, padding: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>권한: {isAdmin ? "ADMIN" : "MANAGER"} {saving ? "· 저장중..." : ""}</div>
          {loading ? <div>불러오는 중...</div> : visibleLogs.map((log) => (
            <div key={log.id} style={{ border: "1px solid #e2e8f0", borderRadius: 8, padding: 10, marginBottom: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {log.hiddenForManager && <span style={{ fontSize: 11, background: "#fef3c7", padding: "2px 8px", borderRadius: 999 }}>MANAGER 숨김</span>}
                  {isAdmin && <button onClick={() => persist(normalizeLogs(logs.map((it) => it.id === log.id ? { ...it, hiddenForManager: !it.hiddenForManager } : it)))} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid #d6d3d1", background: "#fff" }}>{log.hiddenForManager ? "MANAGER에 표시" : "MANAGER에 숨김"}</button>}
                  {isAdmin && <button onClick={() => persist(normalizeLogs(logs.filter((it) => it.id !== log.id)))} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid #fecaca", background: "#fff", color: "#dc2626" }}>로그 삭제</button>}
                </div>
              </div>
