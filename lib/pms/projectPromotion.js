...
export function calculateProjectPromotionCost(item = {}) {
  ...
  if (amounts.finalUnitCost === null || amounts.quantity === null || amounts.quantity <= 0) {
    return { finalUnitCost: null, minimumOrderBatches: null, initialProductionCost: null };
  }
  const finalUnitCost = Math.round(amounts.finalUnitCost * 10) / 10;
  return {
    finalUnitCost,
    minimumOrderBatches: amounts.minimumOrderBatches,
    initialProductionCost: Math.round(finalUnitCost * amounts.minimumOrderQuantity)
  };
}
