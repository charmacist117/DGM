import { calculateBonusPromotion } from "./pricingScenarios.js";
import { getBaseAmounts } from "./distributionAmounts.js";
...
export function remainingYearRatio(date) {
  if (!(date instanceof Date) || Number.isNaN(date.getTime())) return 0;
  const year = date.getFullYear();
  const start = Date.UTC(year, date.getMonth(), date.getDate());
  const yearStart = Date.UTC(year, 0, 1);
  const nextYear = Date.UTC(year + 1, 0, 1);
  return (nextYear - start) / (nextYear - yearStart);
}

export function forecastGrowthYears(mode, startDate, latestMarketYear, offset = 0) {
  const year = startDate.getFullYear();
  const yearStart = new Date(year, 0, 1);
  const nextYear = new Date(year + 1, 0, 1);
  const decimalStartYear = year + ((startDate.getTime() - yearStart.getTime()) / (nextYear.getTime() - yearStart.getTime()));
  const projectionYear = mode === "annual" || offset === 0 ? decimalStartYear + offset : year + offset;
  return Math.max(0, projectionYear - latestMarketYear);
}
...
export function calculateMarketAnalysis(item = {}, rawAnalysis = {}, rawDefaults = {}) {
  ...
  const baseAmounts = getBaseAmounts(sourceItem);
  const baseUnitCost = baseAmounts.finalUnitCost;
  ...
  const batchQuantity = baseAmounts.quantity;
  const minimumOrderBatches = baseAmounts.minimumOrderBatches;
  ...
  const baseDistributionSellingPrice = calculateSellingPriceFromMarginRate(baseUnitCost, marginRate);
  const distributionSellingPrice = pricingScenario?.scenarioType === "bonus"
    ? calculateBonusPromotion({
      unitCost: baseUnitCost, sellingPrice: baseDistributionSellingPrice,
      paidQuantity: pricingScenario.minimumQuantity, bonusQuantity: pricingScenario.bonusQuantity
    }).effectiveUnitPrice
    : baseDistributionSellingPrice;
