...
export function normalizeMarketSizeAnalysis(value = {}) {
  ...
  return {
    ...
    chamyaksaSellingPriceAdjustmentRate: String(source.chamyaksaSellingPriceAdjustmentRate ?? "0"),
    manufacturerSellingPriceAdjustmentRate: String(
      source.manufacturerSellingPriceAdjustmentRate ?? source.supplyPriceAdjustmentRate ?? "0"
    ),
...
function calculatePeriodCagr(populatedYears, periodLength) {
  if (populatedYears.length < periodLength) return null;
...
export function calculateMarketAnalysis(item = {}, rawAnalysis = {}) {
  ...
  const averageMarketKrw = populatedYears.length
    ...
    : null;
  const cagr5Year = calculatePeriodCagr(populatedYears, 5);
  const cagr3Year = calculatePeriodCagr(populatedYears, 3);
