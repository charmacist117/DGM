...
export function supplyItemIdentityKey(item = {}) {
  ...
  return [
    ...
    normalizedText(item.productName),
...
function distributionRecord(item = {}) {
  return {
    ...
    marketDecisionStatus: item.marketDecisionStatus,
    reference: {
...
function promotionRecord(item = {}) {
  return {
    ...
    projectPromotion: isRecord(item.projectPromotion) ? item.projectPromotion : {}
  };
}
...
export function createModuleBackup(moduleType, data = {}, metadata = {}) {
  ...
  if (moduleType === "development") {
    ...
    moduleData = {
      ...
      marketAnalysisDefaults: normalizeMarketAnalysisDefaults(data.marketAnalysisDefaults)
    };
  } else if (moduleType === "promotion") {
    moduleData = {
      promotionItems: (Array.isArray(data.supplyPriceItems) ? data.supplyPriceItems : []).map(promotionRecord)
    };
  } else {
    moduleData = {
      contractRecords: normalizeContractRecords(data.contractRecords)
    };
  }
...
export function parseModuleBackup(value, expectedModuleType) {
  ...
  } else if (value.moduleType === "market") {
    data = {
      marketItems: assertRecords("marketItems", value.data.marketItems),
      marketAnalysisDefaults: normalizeMarketAnalysisDefaults(value.data.marketAnalysisDefaults)
    };
    recordCount = data.marketItems.length;
  } else if (value.moduleType === "promotion") {
    data = { promotionItems: assertRecords("promotionItems", value.data.promotionItems) };
    recordCount = data.promotionItems.length;
  } else {
    data = {
      contractRecords: normalizeContractRecords(assertRecords("contractRecords", value.data.contractRecords))
    };
    recordCount = data.contractRecords.length;
  }
