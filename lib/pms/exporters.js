import { DRAFT_CHECKLIST_FIELDS, normalizeDraftChecklist, normalizeProductSupplySheet } from "@/lib/pms/defaults";
...
export function projectsToSummaryCsv(projects = []) {
  const rows = (Array.isArray(projects) ? projects : []).map((project, index) => {
    ...
    const supplySheet = normalizeProductSupplySheet(project?.productSupplySheet);
    return {
      no: index + 1,
      ...
      manufacturer: supplySheet.manufacturer || project?.manufacturer || "",
      permitCompany: supplySheet.permitCompany || project?.permitCompany || "",
      ingredientName: supplySheet.ingredientName,
      ingredientContent: supplySheet.ingredientContent,
      dosage: supplySheet.dosage,
      efficacy: supplySheet.efficacy,
      supplyUnitPrice: supplySheet.supplyUnitPrice,
      quoteDate: supplySheet.quoteDate,
      attachmentName: supplySheet.attachment?.name || "",
