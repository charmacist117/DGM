...
export function summarizeFullBackupData(data) {
  return {
    ...
    contractRecordCount: data.contractRecords.length,
...
export function createFullBackup(data, metadata = {}) {
  ...
  const compactData = createCompactPmsPayload(validated);
  return {
...
export function validateFullBackupData(value) {
  ...
  const contractRecords = normalizeContractRecords(Array.isArray(value.contractRecords) ? value.contractRecords : []);
  const marketAnalysisDefaults = normalizeMarketAnalysisDefaults(value.marketAnalysisDefaults);
  return { projects, adminLogs, supplyPriceItems, contractRecords, marketAnalysisDefaults };
}
...
export function parseFullBackup(value, { allowLegacy = true } = {}) {
  ...
  if (allowLegacy && Array.isArray(value.projects)) {
    const data = validateFullBackupData({
      ...
      contractRecords: Array.isArray(value.contractRecords) ? value.contractRecords : [],
