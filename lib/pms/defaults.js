...
export const OTC_ETC_CATEGORIES = ["OTC", "ETC"];
export const REGULATORY_DIRECTION_OPTIONS = [
  "유휴허가 판매원등록",
  "기존허가 판매원추가",
  "허가-생산 이원화",
  "허가-생산 단일화",
...
export const EXCLUSIVITY_OPTIONS = ["독점", "비독점"];

export function isOtcEtcCategory(category) {
  return OTC_ETC_CATEGORIES.includes(String(category || "").trim().toUpperCase());
}

export function normalizeRegulatoryDirection(value) {
  return normalizeRegulatoryDirections(value)[0] || "";
}

export function normalizeRegulatoryDirections(value) {
  const source = Array.isArray(value) ? value : [value];
  return [...new Set(
    source
      .map((item) => String(item || "").trim())
      .filter((item) => REGULATORY_DIRECTION_OPTIONS.includes(item))
  )];
}

export function normalizeExclusivityType(value) {
  const raw = String(value || "").trim();
  return EXCLUSIVITY_OPTIONS.includes(raw) ? raw : "";
}
...
export const DEVELOP_SUB_TIMELINE_TEMPLATE = [
  ...
  { id: "dev_production", name: "제품 생산일정", startOffset: 39, duration: 6, enabled: true }
];
...
export function createProjectFromForm({
  ...
  const safeRegulatoryDirections = isOtcEtcCategory(category)
    ? normalizeRegulatoryDirections(
        Array.isArray(regulatoryDirections) && regulatoryDirections.length > 0
          ? regulatoryDirections
          : regulatoryDirection
      )
    : [];
  ...
  return {
    ...
    manager,
    category,
    regulatoryDirection: safeRegulatoryDirections[0] || "",
    regulatoryDirections: safeRegulatoryDirections,
