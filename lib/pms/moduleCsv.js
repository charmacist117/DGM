...
export function supplyModuleToCsv(items = [], categoryLabelById = {}) {
  const headers = [
    "공급단가 ID", "제품명", "카테고리", "제조사", "허가사", "공급 성분", "함량/규격", "원료 원산지",
  ...
  const rows = items.flatMap((item) => {
    ...
    return ingredients.map((ingredient) => [
      ...
      item.permitCompanyFee
        ? (item.permitCompanyFeeRateUnknown ? "알 수 없음(공급단가에 포함)" : `${item.permitCompanyFeeRateType === "margin" ? "허가사 마진율" : "가산수수료율"} ${item.permitCompanyFeeRate}%`)
        : "",
      item.quoteDate,
      item.shelfLife,
      item.memo,
...
export function distributionModuleToCsv(items = []) {
  const headers = [
    "구분", "공급단가 ID", "제품명", "공급 성분/함량", "제조사", "견적일자", "비교 카테고리",
  ...
  items.forEach((item) => {
    ...
    const finalUnitCost = getBaseAmounts(item).finalUnitCost;
    ...
    scenarios.forEach((scenario) => {
      ...
      const bonus = scenario.scenarioType === "bonus" ? calculateBonusPromotion({
        unitCost: finalUnitCost, sellingPrice: calculatedSellingPrice,
        paidQuantity: scenario.minimumQuantity, bonusQuantity: scenario.bonusQuantity
      }) : null;
      const effectiveSellingPrice = bonus ? bonus.effectiveUnitPrice : calculatedSellingPrice;
      const chamyaksaSellingPrice = effectiveSellingPrice === null ? "" : Math.round(effectiveSellingPrice);
      rows.push([
        bonus ? "할증 가격대 (판매가·마진율은 증정 반영 개당 기준)" : "가격대",
        item.id,
        item.productName,
        label,
        item.manufacturer,
        item.quoteDate,
        item.distributionStructure?.comparisonCategory || "",
        bonus ? `${scenario.label} / 유상 1개 기준 ${calculatedSellingPrice === null ? "-" : Math.round(calculatedSellingPrice)}원` : scenario.label,
        bonus ? bonusPromotionQuantityLabel(scenario) : scenario.minimumQuantity,
        bonus ? (bonus.marginRate ?? "") : scenario.chamyaksaMarginRate,
        chamyaksaSellingPrice,
        item.distributionStructure?.pharmacySellingPrice ?? scenario.pharmacySellingPrice,
        "", "", "", "", "", "", "",
        marketDecisionLabel(item.marketDecisionStatus)
      ]);
    });
...
export function marketModuleToCsv(items = [], marketAnalysisDefaults = {}) {
  const headers = [
    "공급단가 ID", "제품명", "공급 성분/함량", "제조사", "포장단위", "배치 당 포장단위 개수",
...
export function projectPromotionModuleToCsv(items = [], projects = []) {
  ...
  const rows = (Array.isArray(items) ? items : [])
    ...
    return [
      item.id, item.productName, ingredientLabel(item), item.category, item.manufacturer, item.permitCompany, item.packagingUnit,
      ...
      promotion.costMemo,
      promotionFinalDecisionLabel(promotion.finalDecision),
      promotion.finalDecisionAt,
      (Array.isArray(promotion.followUps) ? promotion.followUps : []).map((entry) => `${entry.createdAt || ""} | ${promotionProgressLabel(entry.status)} | ${entry.note || ""}`).join("\n"),
