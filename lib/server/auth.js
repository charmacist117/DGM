...
function hasValidSecret() {
  return Buffer.byteLength(getSecret(), "utf8") > 0;
}
...
function encode(payload) {
  if (!hasValidSecret()) throw new Error("AUTH_SECRET is required.");
...
export function getConfiguredLoginAccounts({ includePasswords = false } = {}) {
  if (process.env.PMS_STORAGE_DRIVER === "local-file") {
    return {
      mode: "local-admin",
  ...
  const configuredAccounts = [
    { id: "ADMIN", role: "admin", roleLabel: "관리자", envKey: "APP_ADMIN_PASSWORD", password: process.env.APP_ADMIN_PASSWORD || "" },
    { id: "MANAGER", role: "user", roleLabel: "매니저", envKey: "APP_USER_PASSWORD", password: process.env.APP_USER_PASSWORD || "" }
  ];

  return {
    mode: "password-code",
    authenticationDisabled: false,
    accounts: configuredAccounts.map(({ password, ...account }) => ({
      ...account,
      configured: Boolean(password),
      passwordLength: password.length,
      ...(includePasswords && password ? { password } : {})
    }))
  };
}
