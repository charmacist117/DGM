...
import { compactStorageValue } from "@/lib/pms/storageCompact";
...
function toObjectPayload(value, fallbackId) {
  if (value && typeof value === "object" && !Array.isArray(value)) {
    return compactStorageValue({ ...value, id: value.id ?? fallbackId }, { id: fallbackId });
  }
...
function stripProjectCollections(project) {
  ...
  return compactStorageValue(payload, {});
}
...
export async function saveProjects(projects, adminLogs = [], supplyPriceItems = [], marketAnalysisDefaults = {}, contractRecords = []) {
  ...
  try {
    const updatedAt = await transaction(async (client) => {
      ...
      for (const [projectIndex, sourceProject] of safeProjects.entries()) {
        ...
        await client.query(
          `
            ...
            WHERE pms_projects.sort_order IS DISTINCT FROM EXCLUDED.sort_order
               OR pms_projects.payload IS DISTINCT FROM EXCLUDED.payload;
          `,
          [projectId, projectIndex, JSON.stringify(projectPayload)]
        );

        const usedTaskKeys = new Set();
        const taskEntries = (Array.isArray(project.tasks) ? project.tasks : []).map((sourceTask, taskIndex) => ({
          sourceTask,
          taskIndex,
          taskId: uniqueKey(sourceTask?.id, `task_${taskIndex + 1}`, usedTaskKeys)
        }));
        await client.query(
          "DELETE FROM pms_tasks WHERE project_id = $1 AND NOT (id = ANY($2::text[]));",
          [projectId, taskEntries.map((entry) => entry.taskId)]
        );
        for (const { sourceTask, taskIndex, taskId } of taskEntries) {
          ...
          await client.query(
            `
              ...
              DO UPDATE SET sort_order = EXCLUDED.sort_order, payload = EXCLUDED.payload, updated_at = NOW()
              WHERE pms_tasks.sort_order IS DISTINCT FROM EXCLUDED.sort_order
                 OR pms_tasks.payload IS DISTINCT FROM EXCLUDED.payload;
            `,
        ...
        for (const field of PROJECT_ITEM_FIELDS) {
          ...
          const itemEntries = items.map((sourceItem, itemIndex) => ({
            sourceItem,
            itemIndex,
            itemKey: uniqueKey(sourceItem?.id, `${field}_${itemIndex + 1}`, usedItemKeys)
          }));
          await client.query(
            "DELETE FROM pms_project_items WHERE project_id = $1 AND kind = $2 AND NOT (item_key = ANY($3::text[]));",
            [projectId, field, itemEntries.map((entry) => entry.itemKey)]
          );
          for (const { sourceItem, itemIndex, itemKey } of itemEntries) {
            ...
            await client.query(
              `
                ...
                ON CONFLICT (project_id, kind, item_key)
                DO UPDATE SET sort_order = EXCLUDED.sort_order, payload = EXCLUDED.payload, updated_at = NOW()
                WHERE pms_project_items.sort_order IS DISTINCT FROM EXCLUDED.sort_order
                   OR pms_project_items.payload IS DISTINCT FROM EXCLUDED.payload;
              `,
      ...
      const logEntries = safeAdminLogs.map((sourceLog, logIndex) => ({
        sourceLog,
        logIndex,
        logId: uniqueKey(sourceLog?.id, `admin_log_${logIndex + 1}`, usedLogKeys)
      }));
      await client.query("DELETE FROM pms_admin_logs WHERE NOT (id = ANY($1::text[]));", [logEntries.map((entry) => entry.logId)]);
      for (const { sourceLog, logIndex, logId } of logEntries) {
        ...
        await client.query(
          `
            ...
            ON CONFLICT (id)
            DO UPDATE SET sort_order = EXCLUDED.sort_order, payload = EXCLUDED.payload, updated_at = NOW()
            WHERE pms_admin_logs.sort_order IS DISTINCT FROM EXCLUDED.sort_order
               OR pms_admin_logs.payload IS DISTINCT FROM EXCLUDED.payload;
          `,
      ...
      const supplyEntries = safeSupplyPriceItems.map((sourceItem, itemIndex) => ({
        sourceItem,
        itemIndex,
        itemId: uniqueKey(sourceItem?.id, `supply_price_${itemIndex + 1}`, usedSupplyPriceKeys)
      }));
      await client.query("DELETE FROM pms_supply_price_items WHERE NOT (id = ANY($1::text[]));", [supplyEntries.map((entry) => entry.itemId)]);
      for (const { sourceItem, itemIndex, itemId } of supplyEntries) {
        ...
        await client.query(
          `
            ...
            ON CONFLICT (id)
            DO UPDATE SET sort_order = EXCLUDED.sort_order, payload = EXCLUDED.payload, updated_at = NOW()
            WHERE pms_supply_price_items.sort_order IS DISTINCT FROM EXCLUDED.sort_order
               OR pms_supply_price_items.payload IS DISTINCT FROM EXCLUDED.payload;
          `,
      ...
      await client.query(
        `
          ...
          WHERE pms_state.payload IS DISTINCT FROM EXCLUDED.payload;
        `,
        [SETTINGS_STATE_ID, JSON.stringify(compactStorageValue({
