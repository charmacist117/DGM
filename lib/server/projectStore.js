...
async function seedFromLegacyOrDefaults() {
  ...
  if (legacy.projects.length > 0 || legacy.adminLogs.length > 0) {
    const saved = await saveProjects(legacy.projects, legacy.adminLogs);
    return {
...
export async function loadProjects() {
  ...
  try {
    ...
    const latestProjectUpdate = projectRows.reduce(
      (latest, row) => (new Date(row.updated_at) > new Date(latest) ? row.updated_at : latest),
      projectRows[0].updated_at
    );
    const latestAdminUpdate = adminLogResult.rows.reduce(
      (latest, row) => (new Date(row.updated_at) > new Date(latest) ? row.updated_at : latest),
      latestProjectUpdate
    );
    ...
    return {
      projects,
      adminLogs,
      updatedAt: toIso(latestAdminUpdate),
