...
export async function saveProjects(projects, adminLogs = [], supplyPriceItems = [], marketAnalysisDefaults = {}) {
  await ensureTables();
  ...
  try {
    const updatedAt = await transaction(async (client) => {
      ...
      await client.query(
        ...
        `,
        [SETTINGS_STATE_ID, JSON.stringify({ marketAnalysisDefaults: safeMarketAnalysisDefaults })]
      );
